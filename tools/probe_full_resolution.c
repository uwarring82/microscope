 ...
 * Full-resolution framing diagnostic; writes untouched USB buffers. No firmware writes or kernel driver installation. */
#include <libusb.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc,char **argv) {
    ...
    usleep(300000);
    if(command(h,0xb4,0xc0,0)<0 || command(h,0xb5,0xa0,0)<0 || command(h,0xb7,50,0x35)<0 || command(h,0xb7,1283,9)<0)goto cleanup;
    const int size=((2592*1944+512+511)/512)*512;
    ...
    for(int frame=0;frame<8;frame++){
        ...
        if(r<0 || transferred<2592*1944+512)break;
